...
from dash import ALL, callback_context, dcc, html, page_container
        ...
        dbc.NavItem(
            dbc.DropdownMenu(
                ...
                label=html.Div(
                    [
                        DashIconify(
                            icon="mdi:key-chain", height=20, width=20, className="me-2"
                        ),
...
def generate_settings_offcanvas():
    ...
    return [
                        ...
                        dbc.Accordion(
                            [
                                dbc.AccordionItem(
                                                                ...
                                                                dbc.Label(
                                                                    [
                                                                        ...
                                                                        DashIconify(
                                                                            ...
                                                                            className="ms-2",
                                                                            id="anthropic-api-key-info-icon",
                                                                        ),
                                                                    ]
                                                                ),
                                                                dbc.Popover(
                                                ...
                                                dbc.Col(
                                                    [
                                                        dcc.Link(
                                                            ...
                                                            target="_blank",
                                                            className="halberd-link float-end",
                                                        )
                                                    ]
                                                ),
                                            ]
                                    ...
                                    title="Halberd Attack Agent",
                                    className="mb-4 bg-halberd-dark enhanced-accordion",
                                )
                            ]
                        ),
                    ]
        ...
        dbc.Button(
            [DashIconify(icon="mdi:content-save-cog", className="me-2"), "Save"],
...
app.layout = html.Div(
    [
        dcc.Interval(
            id="interval-to-trigger-initialization-check", interval=60000, n_intervals=0
        ),
        dcc.Interval(
            id="credential-status-update-interval", interval=30000, n_intervals=0
        ),
        ...
        dbc.Modal(
            [
                dbc.ModalHeader(
                    dbc.ModalTitle("Credential Details", className="halberd-brand")
                ),
                ...
                dbc.ModalFooter(
                    [
                        dbc.Button(
                            "Set as Active",
                            id="set-active-credential-btn",
                            className="halberd-button me-2",
                        ),
                        dbc.Button(
                            "Delete Credential",
                            id="delete-credential-btn",
                            className="halberd-button-secondary me-2",
                        ),
                        dbc.Button(
                            "Close",
                            id="close-credential-details-modal",
        ...
        dbc.Modal(
            [
                ...
                dbc.ModalFooter(
                    dbc.Button(
                        "Close",
                        id="close-app-technique-info-display-modal",
                        className="ml-auto",
                    )
                ),
            ],
        ...
        dbc.Modal(
            [
                dbc.ModalHeader(
                    "Error", style={"background-color": "#dc3545", "color": "white"}
                ),
                dbc.ModalBody(id="app-error-display-modal-body"),
                dbc.ModalFooter(
                    dbc.Button(
                        "Close", id="close-app-error-display-modal", className="ml-auto"
                    )
                ),
            ],
        ...
        dbc.Modal(
            [
                dbc.ModalHeader(
                    "Success", style={"background-color": "#28a745", "color": "white"}
                ),
                dbc.ModalBody(id="app-success-display-modal-body"),
                dbc.ModalFooter(
                    dbc.Button(
                        "Close",
                        id="close-app-success-display-modal",
                        className="ml-auto",
                    )
                ),
            ],
...
@app.callback(
    Output(
        component_id="settings-offcanvas",
...
@app.callback(
    Output(
        component_id="anthropic-api-key-fade",
        component_property="is_in",
        allow_duplicate=True,
    ),
    Output(
        component_id="anthropic-api-key-fade",
...
def save_api_key_to_env_dotenv(n_clicks, api_key_value):
    ...
    if not api_key_value or api_key_value.strip() == "":
        return (
            True,
            html.Div("Please enter a valid API key", className="text-danger"),
            "",
        )
    ...
    try:
        ...
        return (
            True,
            html.Div("API key saved successfully!", className="text-success"),
            "",
        )
        ...
        return (
            True,
            html.Div(f"Error saving API key: {str(e)}", className="text-danger"),
            "",
        )
...
def set_active_credential_from_modal(n_clicks):
    ...
    try:
        message = CredentialManager.set_active_credential(
            current_modal_provider, current_modal_credential
        )
        return True, message, "Success", "success", False
    except Exception as e:
        return (
            True,
            f"Failed to set active credential: {str(e)}",
            "Error",
            "danger",
            False,
        )
...
def delete_credential_from_modal(n_clicks):
    ...
    try:
        message = CredentialManager.delete_credential(
            current_modal_provider,
            current_modal_credential.id,
            current_modal_credential,
        )
        return True, message, "Success", "success", False
    except Exception as e:
...
@app.callback(
    ...
    [
        Input(
            {
                "type": "delete-expired-credential",
                "credential_id": ALL,
                "provider": ALL,
            },
            "n_clicks",
        ...
        Input(
            {"type": "delete-credential", "credential_id": ALL, "provider": ALL},
            "n_clicks",
        ),
    ],
    prevent_initial_call=True,
