...
from dash import ALL, callback, clientside_callback, dcc, html, register_page
...
from core.Functions import (
    GetAllPlaybooks,
    ParseTechniqueResponse,
    generate_attack_tactics_options,
    generate_attack_technique_config,
    generate_attack_technique_options,
    generate_aws_access_info,
    generate_azure_access_info,
    generate_entra_access_info,
    generate_gcp_access_info,
    generate_technique_info,
)
from core.gcp.gcp_access import GCPAccess
from core.logging.logger import StructuredAppLog, app_logger
...
layout = html.Div(
    [
        dbc.Row(
            [
                dbc.Col(
                    ...
                    md=4,
                    className="enhanced-sidebar-col",
                    style={
                        "height": "70vh",
                        "overflowY": "auto",
                        "position": "relative",
        ...
        dbc.Modal(
            [
                ...
                dbc.ModalBody(
                    id="attack-access-info-display-modal-body",
                    className="enhanced-attack-surface-card halberd-depth-card",
                    style={
                        "background": "linear-gradient(135deg, rgba(33, 37, 41, 0.98) 0%, rgba(52, 58, 64, 0.98) 100%)",
                        "border": "1px solid rgba(220, 53, 69, 0.2)",
                        "borderRadius": "12px",
    ...
    style={"minHeight": "100vh", "padding-right": "20px", "padding-left": "20px"},
)
...
@callback(
    Output(
        component_id="technique-config-display-file-name", component_property="children"
    ),
    ...
    if isinstance(output, tuple) and len(output) == 2:
        ...
        if result.value == "success":
            ...
            output_manager.store_technique_output(
                data=response["value"], technique_name=t_id, event_id=event_id
            )

            # Return results
            return (
                ParseTechniqueResponse(response["value"]),
                True,
                "Technique Execution Successful",
            )
        ...
        # Save output to file
        output_manager.store_technique_output(
            data=response["error"], technique_name=t_id, event_id=event_id
        )
        # Return results
        return (
            ParseTechniqueResponse(response["error"]),
            True,
...
@callback(
    Output(
        component_id="attack-technique-info-div",
        component_property="children",
        allow_duplicate=True,
    ),
    Input(component_id="attack-options-radio", component_property="value"),
...
@callback(
    ...
    Input(
        component_id="interval-to-trigger-initialization-check",
...
def set_entra_active_token_callback(value):
    ...
    for token in manager.get_all_tokens():
        ...
        if token_info is not None:
            if (
                token_info["Entity"] == selected_token_entity
                and token_info["Access Exp"] == selected_token_exp
            ):
                access_token = token
                break
        else:
...
def generate_entra_token_dropdown_callback(title):
    ...
    if title is None:
        ...
        for token in manager.get_all_tokens():
            ...
            if token_info is not None:
                selected_value = {
                    token_info.get("Entity"): token_info.get("Access Exp")
                }
                all_tokens.append(
                    {
                        "label": html.Div(
                            f"{token_info['Entity']}-{token_info.get('Access Exp')}",
                            className="halberd-text",
                        ),
...
@callback(
    ...
    Input(
        component_id="azure-subscription-selector-dropdown", component_property="value"
    ),
)
...
@callback(
    Output(
        component_id="azure-subscription-selector-dropdown",
        component_property="options",
    ),
    Input(
        component_id="azure-subscription-selector-dropdown", component_property="title"
    ),
)
...
@callback(
    ...
    State(component_id="attack-options-radio", component_property="value"),
    State(
        component_id={"type": "technique-config-display", "index": ALL},
        component_property="value",
    ),
    State(
        component_id={"type": "technique-config-display-boolean-switch", "index": ALL},
        component_property="on",
    ),
    State(
        component_id={"type": "technique-config-display-file-upload", "index": ALL},
        component_property="contents",
    ),
    prevent_initial_call=True,
)
def add_technique_to_pb_callback(
    n_clicks, selected_pb, step_no, wait, t_id, values, bool_on, file_content
):
    if n_clicks == 0:
        raise PreventUpdate
    ...
    if session_name is None:
        ...
        for session in manager.list_sessions():
            all_sessions.append(
                {
                    "label": html.Div(
                        session["session_name"], className="halberd-text"
                    ),
...
@callback(
    Output(
        component_id="gcp-credential-selector-dropdown", component_property="options"
    ),
...
@callback(
    Output(
        component_id="attack-access-info-dynamic-btn", component_property="children"
    ),
    Output(component_id="attack-access-info-dynamic-btn", component_property="color"),
    Input(component_id="attack-surface-tabs", component_property="active_tab"),
    Input(
        component_id="attack-access-info-display-modal", component_property="is_open"
    ),  # refresh button status automatically
)
    ...
    if active_tab in ["tab-attack-EntraID", "tab-attack-M365"]:
        ...
        if access_token:
            ...
            if access_info is not None:
                if access_info["Access Exp"] < datetime.datetime.fromtimestamp(
                    int(time.time()), tz=datetime.timezone.utc
...
def display_access_info_in_modal_callback(n_clicks, active_tab):
    ...
    if active_tab in ["tab-attack-EntraID", "tab-attack-M365"]:
        return True, create_access_section(
            "token-selector-dropdown", "del-entra-token-button", "access-info-div"
        )
    elif active_tab == "tab-attack-AWS":
        return True, create_access_section(
...
@callback(
    [Output("config-collapse", "is_open"), Output("config-arrow", "className")],
