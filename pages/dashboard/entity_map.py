...
def generate_entity_mapping_graph(map_layout="cose", filter_category=None):
    ...
    if access_type == "user":
        ...
        elements.append(
            {"data": {"id": "0", "label": displayname}, "classes": "entity"}
        )
        ...
        for category in categories:
            ...
            elements.append(
                {"data": {"id": str(n), "label": category}, "classes": "category"}
            )
            # Create an edge between entity and each detection
            elements.append({"data": {"source": "0", "target": str(n)}})
            n += 1
            if category == "Groups":
                if ListJoinedTeams() is not None:
                    sub_node_n = n - 1
                    teams = ListJoinedTeams()
                    for team in teams:
                        elements.append(
                            {
                                "data": {"id": str(n), "label": team},
                                "classes": "end_node",
                            }
                        )
                        # Create an edge between entity and each detection
                        elements.append(
                            {"data": {"source": str(sub_node_n), "target": str(n)}}
                        )
                        n += 1
            if category == "Roles":
                if ListObjectsMemberOf() is not None:
                    sub_node_n = n - 1
                    group_list, roles_list, others_list = ListObjectsMemberOf()
                    for role in roles_list:
                        elements.append(
                            {
                                "data": {"id": str(n), "label": role},
                                "classes": "end_node",
                            }
                        )
                        # Create an edge between entity and each detection
            ...
            if category == "Applications":
                if ListAppRoleAssignments() is not None:
                    sub_node_n = n - 1
                    applications = ListAppRoleAssignments()
                    for application in applications:
                        elements.append(
                            {
                                "data": {"id": str(n), "label": application},
                                "classes": "end_node",
                            }
                        )
                        # Create an edge between entity and each detection
            ...
            [
                ...
                dbc.Col(
                    [
                        dbc.Button(
                            "Generate Entity Map",
                            id="generate-entity-map-button",
                            className="me-3 halberd-button",
                        ),
                        dbc.Button(
                            "Export Results",
                            color="primary",
                            className="me-3 halberd-button-secondary",
                            id="export-button",
                        ),
                    ],
                    width=6,
        ...
        dbc.Col(
            [
                dbc.Card(
                    [
                        dbc.CardHeader(
                            "Entity Map", className="bg-halberd-dark text-light"
                        ),
                        dbc.CardBody(
                            [
                                dcc.Loading(
                ...
                dbc.Card(
                    [
                        dbc.CardHeader(
                            "Node Info", className="bg-halberd-dark text-light"
                        ),
                        dbc.CardBody(
                            [
                                html.Div(
                                    id="entity-map-node-info-div",
