...
from .credential_types import PROVIDER_CONFIGS, CredentialData, ProviderConfig
...
class CredentialManager:
    ...
    def get_all_credentials(
        ...
        List[Tuple[CredentialData, str, bool]], List[Tuple[CredentialData, str, bool]]
    ]:
        ...
        for provider_name, provider in cls._providers.items():
            try:
                ...
                for credential in credentials:
                    ...
                    target_list.append(
                        (credential, provider_name, credential.is_active)
                    )
            except Exception:
                continue
    ...
    def delete_credential(
        ...
        try:
            return cls._providers[provider_name].delete_credential(
    ...
    def set_active_credential(
        cls, provider_name: str, credential_data: CredentialData
    ) -> str:
    ...
    def create_credential_item(
        ...
        config = PROVIDER_CONFIGS.get(
            provider_name,
            ProviderConfig(
                icon="mdi:key", color="gray", display_name=provider_name.upper()
            ),
        )

        # Provider icon with appropriate color
        provider_color = "gray" if is_expired else config.color
        provider_icon = DashIconify(
            icon=config.icon, className="me-2", style={"color": provider_color}
        )
        ...
        if not is_expired and is_active:
            status_elements.append(
                DashIconify(icon="mdi:check-circle", className="text-success me-2")
            )
        elif is_expired:
            status_elements.append(
                DashIconify(icon="mdi:alert-circle", className="text-danger me-2")
            )

        # Name and active indicator
        name_class = "text-muted" if is_expired else ""
        name_element = (
            html.Span(f"{provider_name.upper()}: {name}", className=name_class)
            if is_expired
            else html.Strong(f"{provider_name.upper()}: {name}")
        )
        ...
        detail_text = (
            f"Expired: {detail}"
            if is_expired and detail
            else detail
            if detail
            else ("Expired" if is_expired else None)
        )
                    ...
                    [
                        html.Div(
                            ...
                            + [
                                name_element,
                                active_span,
                                html.Div([delete_btn], className="ms-auto"),
                            ],
                            className="mb-1 d-flex align-items-center",
                        ),
                        html.Small(detail_text, className=detail_class)
                        if detail_text
                        else None,
                    ],
                    className="p-2",
        ...
        return dbc.DropdownMenuItem(
            item_content,
            className="credential-dropdown-item p-0",
    ...
    def get_dropdown_content(cls) -> Tuple[str, List[dbc.DropdownMenuItem]]:
        ...
        try:
            ...
            for credential_data, provider_name, is_active in valid_credentials:
                item = cls.create_credential_item(
                    credential_data, provider_name, is_active, False
                )
            ...
            for credential_data, provider_name, is_active in expired_credentials:
                item = cls.create_credential_item(
                    credential_data, provider_name, is_active, True
                )
                dropdown_items.append(item)
            ...
            if not dropdown_items:
                dropdown_items = [
                    dbc.DropdownMenuItem("No credentials available", disabled=True)
                ]

            # Create count display
            total_count = len(valid_credentials) + len(expired_credentials)
            count_text = (
                f"{len(valid_credentials)}/{total_count}" if total_count > 0 else "0"
            )
