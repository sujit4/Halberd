...
import pandas as pd
import yaml
from dash import Patch, dash_table, dcc, html
...
def generate_technique_info(technique_id) -> list:
                    ...
                    dbc.CardBody(
                        [
                                    ...
                                    dbc.Col(
                                        [
                                            dbc.Button(
                                                ...
                                                href=mitre_info.mitre_url
                                                if mitre_info.mitre_url
                                                not in [None, "#"]
                                                else "#",
                                                target="_blank",
                            ...
                            html.Div(
                                ...
                                if mitre_info.sub_technique_name
                                and mitre_info.sub_technique_name != "None"
                                else []
                            ),
                            # Tactics section
                            html.Div(
                                [
                                    html.Small(
                                        "Tactics:", className="text-muted d-block mb-1"
                                    ),
                                    html.Div(
                                        tactic_badges, className="d-flex flex-wrap"
                                    ),
                                ]
                            ),
                        ]
                    ...
                    dbc.CardBody(
                        [
                                    ...
                                    dbc.Col(
                                        [
                                            html.Div(
                                                [
                                                    html.I(
                                                        className="fab fa-microsoft me-2",
                                                        style={"color": "#0078d4"},
                                                    ),
                                                    html.Strong(
                                    ...
                                    dbc.Col(
                                        [
                                            dbc.Button(
                                                ...
                                                href=azure_trm_info.azure_trm_url
                                                if azure_trm_info.azure_trm_url
                                                not in [None, "#"]
                                                else "#",
                                                target="_blank",
                                                color="outline-info",
                            ...
                            html.Div(
                                ...
                                if azure_trm_info.sub_technique_name
                                and azure_trm_info.sub_technique_name != "None"
                                else []
                            ),
                            # Tactics section
                            html.Div(
                    ...
                    dbc.Row(
                        [
                            ...
                            dbc.Col(
                                ...
                                lg=4,
                                md=12,
                                className="text-end mt-2 mt-lg-0",
                            ),
                        ],
                        align="center",
                    )
        ...
        if technique.azure_trm_techniques:
            modal_content.append(
                dbc.Accordion(
                    [
                        dbc.AccordionItem(
                            create_azure_trm_info_cards(technique.azure_trm_techniques),
                            title="Azure Threat Research Matrix Reference",
                        )
                    ],
    ...
    if technique.notes:
        ...
        modal_content.append(
            dbc.Accordion(
                [dbc.AccordionItem(notes_content, title="Technique Notes")],
                start_collapsed=True,
                className="mb-3 enhanced-accordion",
    ...
    if technique.references:
        ...
        modal_content.append(
            dbc.Accordion(
                [dbc.AccordionItem(references_content, title="Technique References")],
                start_collapsed=True,
                className="mb-3 enhanced-accordion",
                style={"overflow": "hidden"},
            )
...
def GetAllPlaybooks():
    ...
    try:
        ...
        for content in dir_contents:
            if os.path.isfile(
                os.path.join(AUTOMATOR_PLAYBOOKS_DIR, content)
            ) and content.lower().endswith(".yml"):
...
def ParseTechniqueResponse(technique_response):
    ...
    def format_output(
        data: Any, level: int = 0
    ) -> Union[dbc.Card, dbc.ListGroup, html.Span]:
        """
    ...
    def create_cards(data: Any) -> Union[dbc.Card, list]:
        ...
        if isinstance(data, dict):
            return [
                dbc.Card(
                    dbc.CardBody(
                        [html.H5(key, className="card-title"), format_output(value)]
                    ),
                    className="mb-3",
                )
                for key, value in data.items()
...
def playbook_viz_generator(playbook_name: Optional[str]) -> html.Div:
    ...
    if playbook_name is None:
        ...
        used_surfaces = {
            node["data"]["category"]
            for node in attack_sequence_viz_elements
            ...
            if tactic in mitre_technique["tactics"]:
                if technique_module not in technique_tracker:
                    ...
                    technique_options_list.append(
                        {
                            "label": html.Div(
                                [technique().name], className="halberd-brand"
                            ),
...
def generate_attack_technique_config(technique):
                        ...
                        [
                            html.Label(
                                [
                                    ...
                                    if input_config.get("description")
                                    else "",
                                ],
                    ...
                    html.Div(
                        [
                            ...
                            dbc.Input(
                                ...
                                placeholder=f"Default: {input_config['default']}"
                                if input_config["default"]
                                else f"Enter {input_config['name'].lower()}...",
                                debounce=True,
                                id={
                                    "type": "technique-config-display",
                                    "index": input_field,
                                },
                            ...
                            if input_config["input_field_type"]
                            ...
                            # Select Dropdown
                                ...
                                placeholder=f"Default: {input_config['default']}"
                                if input_config["default"]
                                else f"Select {input_config['name'].lower()}...",
                            ...
                            if input_config["input_field_type"] == "select"
                            # Boolean Switch
                            else html.Div(
                                [
                                    daq.BooleanSwitch(
                                        id={
                                            "type": "technique-config-display-boolean-switch",
                                            "index": input_field,
                                        },
                            ...
                            if input_config["input_field_type"] == "bool"
                            # File Upload
                            else html.Div(
                                [
                                    dcc.Upload(
                                        id={
                                            "type": "technique-config-display-file-upload",
                                            "index": input_field,
                                        },
                                        children=html.Div(
                                            [
                                                html.Div(
                                                    [
                                                        ...
                                                        html.I(
                                                            id="technique-config-display-file-name",
                                                            style={
                                                                "marginTop": "8px",
                                                                "color": "#fff",
                                                                "fontSize": "0.85rem",
                            ...
                            if input_config["input_field_type"] == "upload"
                            else html.Div()
                        ],
                ...
                [
                    html.Div(
                        [
                            ...
                            html.P(
                                "This technique requires no additional configuration.",
    ...
    action_section = html.Div(
        ...
        style={"borderRadius": "0 0 12px 12px", "backdropFilter": "blur(10px)"},
    )
            ...
            [
                                        ...
                                        dbc.Col(
                                            [
                                                html.Label(
                                                    [
                                                        html.I(
                                                            className="fas fa-sort-numeric-up me-2",
                                                            style={
                                                                "color": "#6c757d",
                                                                "fontSize": "0.9rem",
                                                            },
                ...
                dbc.ModalFooter(
                    [
                        ...
                        dbc.Button(
                            [html.I(className="fas fa-plus me-2"), "Add to Playbook"],
...
def generate_entra_access_info(access_token):
    def create_scope_badges(scopes):
        return [
            dbc.Badge(scope, color="dark", className="me-1 mb-1 small")
            for scope in scopes
        ]
                ...
                dbc.Col(
                    [
                        html.Div(
                            [
                                DashIconify(
                                    icon="mdi:identifier", className="me-2"
                                ),  # Icon for entity
                                html.Strong("Entity:"),
                                html.Span(
                                    access_info.get("Entity", ""),
                        ...
                        html.Div(
                            [
                                DashIconify(
                                    icon=entity_type_icon, className="me-2"
                                ),  # Dynamic icon based on entity type
                                html.Strong("Entity Type:"),
                                html.Span(
                                    access_info.get("Entity Type", ""), className="ms-2"
                                ),
                            ],
                            className="mb-2",
                        ),
                        html.Div(
                            [
                                DashIconify(
                                    icon="mdi:key-variant", className="me-2"
                                ),  # Key icon
                                html.Strong("Access Type:"),
                                html.Span(
                                    access_info.get("Access Type", ""), className="ms-2"
                                ),
                            ],
                            className="mb-2",
                        ),
                ...
                dbc.Col(
                    [
                        html.Div(
                            [
                                DashIconify(
                                    icon="mdi:clock-outline", className="me-2"
                                ),  # Clock icon for token expiration time
                                html.Strong("Access Exp:"),
                                html.Span(
                                    f"{access_info.get('Access Exp', '')} UTC",
                                    className="ms-2",
                                ),
                                html.Span("Expired", className="ms-2 text-danger")
                                if access_info.get("Access Exp", "")
                                < datetime.datetime.now(datetime.timezone.utc).strftime(
                                    "%Y-%m-%dT%H:%M:%SZ"
                                )
                                else html.Span("Valid", className="ms-2 text-success"),
                            ],
                            className="mb-2",
                        ),
                        html.Div(
                            [
                                DashIconify(
                                    icon="mdi:domain", className="me-2"
                                ),  # Icon for tenant identifier
                                html.Strong("Target Tenant:"),
                                html.Span(
                                    access_info.get("Target Tenant", ""),
                                    className="ms-2",
                                ),
                            ],
                            className="mb-2",
                        ...
                        html.Div(
                            [
                                DashIconify(
                                    icon="mdi:settings-applications", className="me-2"
                                ),  # Icon for tenant identifier
                                html.Strong("Target App:"),
                                html.Span(
                                    access_info.get("Target App Name", ""),
                                    className="ms-2",
                                ),
                            ],
                            className="mb-2",
                ...
                dbc.Col(
                    [
                        html.Div(
                            [
                                DashIconify(
                                    icon="mdi:shield-check", className="me-2"
                                ),  # Shield icon for access scope
                                html.Strong("Access scope:"),
                            ],
                            className="mb-2",
                        ),
                        html.Div(
                            create_scope_badges(access_info.get("Access scope", "")),
    ...
    return dbc.Card(
        dbc.CardBody(
            [
                html.H4(
                    [
                        DashIconify(icon="mdi:key-chain", className="me-2"),
                        ...
                        dbc.Col(
                            [
                                html.Div(
                                    [
                                        DashIconify(
                                            icon="mdi:account-key", className="me-2"
                                        ),
                                        html.Strong("User ID:"),
                                        html.Span(
                                            session_info["UserId"],
                                ...
                                html.Div(
                                    [
                                        DashIconify(
                                            icon="mdi:account-cash", className="me-2"
                                        ),
                                        html.Strong("Account:"),
                                        html.Span(
                                            session_info["Account"], className="ms-2"
                                        ),
                                    ],
                                    className="mb-3",
                                ),
                                html.Div(
                                    [
                                        ...
                                        html.Strong("ARN:"),
                                        html.Span(
                                            session_info["Arn"], className="ms-2"
                                        ),
                                    ],
                                    className="mb-3",
                                ),
                        ...
                        [
                            html.H4(
                                [
                                    ...
                                    "Access: ",
                                    html.Span(
                                        "VALID SESSION", className="text-success"
                                    ),
                                ],
                                className="card-title mb-3",
                            ),
                        ...
                        [
                            html.H4(
                                [
                                    ...
                                    "Access: ",
                                    html.Span(
                                        "NO VALID SESSION", className="text-danger"
                                    ),
                                ],
...
def generate_gcp_access_info(credential_name):
    ...
    if credential_name:
        ...
        raw_credential = manager.get_detailed_credential(data=b64str_credential).get(
            "credential"
        )
        try:
            manager = GCPAccess(raw_credential, name=credential_name)

            if not manager.get_expired_info() and manager.get_validation():
                                ...
                                [
                                    ...
                                    html.Div(
                                        [
                                            ...
                                            html.Strong("Project:"),
                                            html.Span(
                                                credential.project_id, className="ms-2"
                                            ),
                                        ],
                                        className="mb-3",
                                    ),
                                    html.Div(
                                        [
                                            DashIconify(
                                                icon="mdi:telescope", className="me-2"
                                            ),
                                            html.Strong("Scopes:"),
                                            html.Span(
                                                credential.scopes, className="ms-2"
                                            ),
                                        ],
                                        className="mb-3",
                                    ),
                        ...
                        dbc.CardBody(
                            [
                                html.H4(
                                    [
                                        DashIconify(
                                            icon="mdi:google-cloud", className="me-2"
                                        ),
                                        "Access: ",
                                        html.Span(
                                            "VALID SESSION", className="text-success"
                    ...
                    [
                        html.H4(
                            [
                                ...
                                "Access: ",
                                html.Span(
                                    "CREDENTIAL ARE NOT SET", className="text-danger"
                                ),
                            ],
                            className="card-title",
                        )
                        ...
                        dbc.Col(
                            [
                                ...
                                html.Div(
                                    [
                                        DashIconify(
                                            icon="mdi:flag-variant", className="me-2"
                                        ),
                                        html.Strong("Is Default:"),
                                        html.Span(
                                            str(current_access.get("isDefault", "N/A")),
                                            className="ms-2",
                                        ),
                                    ],
                                    className="mb-2",
                        ...
                        dbc.Col(
                            [
                                html.Div(
                                    [
                                        DashIconify(
                                            icon="mdi:state-machine", className="me-2"
                                        ),
                                        html.Strong("State:"),
                                        html.Span(
                                            current_access.get("state", "N/A"),
                                ...
                                html.Div(
                                    [
                                        ...
                                        html.Span(
                                            current_access.get("tenantId", "N/A"),
                                            className="ms-2",
                                        ),
                                    ],
                                    className="mb-2",
                                ...
                                html.Div(
                                    [
                                        ...
                                        html.Span(
                                            current_access.get("homeTenantId", "N/A"),
                                            className="ms-2",
                                        ),
                                    ],
                                    className="mb-2",
                    ...
                    dbc.CardBody(
                        [
                            html.H4(
                                [
                                    DashIconify(
                                        icon="mdi:microsoft-azure", className="me-2"
                                    ),
                                    "Access: ",
                                    html.Span(
                                        "ACTIVE SESSION", className="text-success"
                                    ),
                                ],
                                className="card-title mb-3",
                            ),
                        ...
                        [
                            html.H4(
                                [
                                    ...
                                    "Access: ",
                                    html.Span(
                                        "NO ACTIVE SESSION", className="text-danger"
                                    ),
                                ],
                                className="card-title",
                            )
...
def create_app_log_event_summary(grouped_events):
    ...
    for event_id, events in grouped_events.items():
        ...
        end_event = next(
            (e for e in events if e["status"] in ["completed", "failed"]), None
        )
                ...
                columns=[
                    {
                        "name": i,
                        "id": i,
                        "presentation": "markdown" if i == "Output" else None,
                    }
                    for i in df.columns
                ],
                data=df.to_dict("records"),
                style_table={"overflowX": "auto", "backgroundColor": "#2F4F4F"},
                style_data={
...
def get_playbook_stats():
    ...
    try:
        ...
        return {
            ...
            "last_sync": datetime.datetime.fromtimestamp(last_modified)
            if last_modified
            else None,
        }
        ...
        return {"total_playbooks": 0, "last_sync": None}
