...
class M365ExfilUserMailbox(BaseTechnique):
    def __init__(self):
        ...
        super().__init__(
            "Exfil Users Mailbox",
            "Perform email reconnaissance leveraging Microsoft Graph API to exfiltrate emails from a target user's mailbox. Technique can perform targeted searches across email fields (body, subject, attachments, sender) and extract sensitive information including sender details, recipients, subjects, and message previews - making it useful for both broad mailbox enumeration and focused data collection. Warning: Attempting to exfiltrate entire mailbox can take really long time.",
            mitre_techniques,
        )
    ...
    def execute(self, **kwargs: Any) -> Tuple[ExecutionStatus, Dict[str, Any]]:
        ...
        try:
                ...
                if search_field:
                    if search_field not in search_field_options:
                        return ExecutionStatus.FAILURE, {
                            ...
                            "message": {
                                "Error": f"Invalid Technique Input - Search Field must be in {search_field_options}"
                            },
                        }
            ...
            if email_collected:
                ...
                return ExecutionStatus.SUCCESS, {
                    "message": "No emails found in users mailbox",
                    "value": [],
                }
