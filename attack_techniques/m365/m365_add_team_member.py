...
class M365AddTeamMember(BaseTechnique):
    def __init__(self):
        ...
        technique_refs = [
            TechniqueReference(
                "Microsoft Graph API - Add Member to Team",
        ...
        technique_notes = [
            TechniqueNote(
                "Ensure the account being used to execute this technique has sufficient permissions to add members to the target team."
            ),
            TechniqueNote(
                "For internal users, the specified user must already exist in the organization's directory."
            ),
            TechniqueNote(
                "This technique requires you to know the team's ID, which can be obtained using the 'M365 Enumerate Teams' technique."
            ),
            TechniqueNote(
                "The newly added member will have the default role of 'member' unless specified otherwise."
            ),
            TechniqueNote(
                "Consider using this technique in conjunction with other M365 techniques to establish persistence across multiple services."
            ),
        ]
        ...
        try:
            ...
            if user_identifier in [None, ""]:
                return ExecutionStatus.FAILURE, {
                    "error": {"Error": "Invalid Technique Input"},
                    "message": {
                        "Error": "User identifier (ID, UPN, or email) is required"
                    },
                }
