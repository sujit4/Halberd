...
class M365SearchOutlookMessages(BaseTechnique):
    def __init__(self):
        ...
        super().__init__(
            ...
            "Searches outlook to collect data",
            mitre_techniques,
        )
        ...
        try:
            ...
            if 200 <= raw_response.status_code < 300:
                    ...
                    for hits in search_match["hitsContainers"]:
                        if hits.get("total", 0) > 0:
                            for hit in hits["hits"]:
                                output.append(
                                    {
                                        "subject": hit.get("resource", "N/A").get(
                                            "subject", "N/A"
                                        ),
                                        "preview": hit.get("resource", "N/A").get(
                                            "bodyPreview", "N/A"
                                        ),
                                        "summary": hit.get("summary", "N/A"),
                                        "sender": hit.get("resource", "N/A")
                                        .get("sender", "N/A")
                                        .get("emailAddress", "N/A"),
                                        "reply_to": hit.get("resource", "N/A").get(
                                            "replyTo", "N/A"
                                        ),
                                        "has_attachments": hit.get(
                                            "hasAttachments", "N/A"
                                        ),
                                    }
                                )
                if output:
                    ...
                    return ExecutionStatus.SUCCESS, {
                        "message": "No outlook messages found",
                        "value": [],
                    }
            else:
                return ExecutionStatus.FAILURE, {
