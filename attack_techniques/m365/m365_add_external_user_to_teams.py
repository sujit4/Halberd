...
class M365AddExternalUserToTeam(BaseTechnique):
    def __init__(self):
        ...
        technique_notes = [
            TechniqueNote(
                "Before executing, use the 'Enumerate Teams' technique to identify target teams"
            ),
            TechniqueNote("The Team ID can be found in the Enumerate Teams results"),
            TechniqueNote(
                "Use organizational email formats for external invitations to appear legitimate"
            ),
            TechniqueNote(
                "Guest users have limited permissions by default, while member and owner roles have progressively more access"
            ),
            TechniqueNote(
                "The invitation URL can be shared directly with the target user to bypass email notification"
            ),
        ]
        ...
        technique_refs = [
            ...
            TechniqueReference(
                "Microsoft Graph API - Add team member",
                "https://learn.microsoft.com/en-us/graph/api/team-post-members",
            ),
            TechniqueReference(
                "Microsoft Graph API - Create invitation",
    ...
    def execute(self, **kwargs: Any) -> Tuple[ExecutionStatus, Dict[str, Any]]:
        ...
        try:
            ...
            invitation_response = GraphRequest().post(
                url=invitation_url, data=invitation_data
            )
            ...
            if 200 <= invitation_response.status_code < 300:
                ...
                invited_user_id = (
                    invitation_response.json().get("invitedUser", {}).get("id")
                )
                ...
                team_add_response = GraphRequest().post(
                    url=endpoint_url, data=team_add_data
                )
                ...
                if 200 <= team_add_response.status_code < 300:
                    return ExecutionStatus.SUCCESS, {
                        ...
                        "value": {
                            ...
                            "user_email": external_email,
                            "display_name": team_add_response.json().get(
                                "displayName", "N/A"
                            ),
                            "user_id": invited_user_id,
                            "team_id": team_id,
                            "role": team_add_response.json().get("roles", []),
                            "visibility_start_date": team_add_response.json().get(
                                "visibleHistoryStartDateTime", "N/A"
                            ),
                            "tenant_id": team_add_response.json().get(
                                "tenantId", "N/A"
                            ),
                            "invitation_url": invitation_response.json().get(
                                "inviteRedeemUrl"
                            ),
                        },
                    }
                    ...
                    return ExecutionStatus.FAILURE, {
                        "message": "User invited but failed to add to team",
                        "error": {
                            "value": {
                                "user_email": external_email,
                                "user_id": invited_user_id,
