...
class AzureEnumerateKeyVaults(BaseTechnique):
    def __init__(self):
        ...
        notes = [
            TechniqueNote(
                "This technique is useful as part of a broader discovery strategy to identify potential sensitive credential stores"
            ),
            TechniqueNote(
                "Key Vaults often contain high-value targets like service credentials, certificates, and connection strings"
            ),
            TechniqueNote(
                "Consider following up with AzureDumpKeyVault technique to extract vault contents"
            ),
            TechniqueNote(
                "Check vault access policies and RBAC settings for potential access vectors"
            ),
        ]
            ...
            for sub_id in subscriptions_to_scan:
                ...
                try:
                    ...
                    subscription_info = next(
                        (
                            sub
                            for sub in AzureAccess().get_account_available_subscriptions()
                            if sub.get("id") == sub_id
                        ),
                        None,
                    )
                    subscription_name = (
                        subscription_info.get("name") if subscription_info else sub_id
                    )
                    ...
                    if resource_group_name:
                        ...
                        resource_groups = [
                            rg.name for rg in res_client.resource_groups.list()
                        ]
                            ...
                            for vault in vaults:
                                ...
                                if include_details:
                                    ...
                                    if (
                                        ...
                                        and vault.properties.private_endpoint_connections
                                    ):
