...
class AzureMassResourceDeletion(BaseTechnique):
    def __init__(self):
        ...
        references = [
            TechniqueReference(
                "Azure Resource Manager API Reference",
                "https://learn.microsoft.com/en-us/rest/api/resources/resources/delete",
            ),
            TechniqueReference(
                "Azure Resource Manager resource group and resource deletion",
                "https://docs.azure.cn/en-us/azure-resource-manager/management/delete-resource-group?tabs=azure-python",
            ),
            TechniqueReference(
        ...
        notes = [
            TechniqueNote(
                "This technique permanently deletes resources and should only be used in controlled test environments"
            ),
            TechniqueNote(
                "Resource deletion can trigger alerts in security monitoring systems"
            ),
            TechniqueNote(
                "Some resources have deletion locks that may prevent this technique from succeeding"
            ),
            TechniqueNote(
                "Critical resources may have additional protection policies requiring additional authentication"
            ),
            TechniqueNote(
                "The 'wait_for_completion' parameter allows for synchronous deletion when set to True"
            ),
            TechniqueNote(
                "Using resource_type_filter can target specific resource categories"
            ),
        ]
        super().__init__(
    ...
    def _delete_resource(
        self, resource_client, resource_id: str, wait_for_completion: bool
    ) -> Tuple[str, bool, str]:
    ...
    def execute(self, **kwargs: Any) -> Tuple[ExecutionStatus, Dict[str, Any]]:
        ...
        try:
            ...
            max_resources: int = kwargs.get(
                "max_resources", 10
            )  # Default limit to prevent unintended mass deletion
            ...
            for resource in resources:
                ...
                if (
                    ...
                    and resource_type_filter.lower() not in resource.type.lower()
                ):
                    continue
                ...
                for future in concurrent.futures.as_completed(future_to_resource):
                    ...
                    if success:
                        deleted_resources.append(
                            {"resource_id": resource_id, "message": message}
                        )
                    else:
                        failed_resources.append(
                            {"resource_id": resource_id, "error": message}
                        )
