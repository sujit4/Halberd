            ...
            for vault in client.vaults.list():
                ...
                try:
                    ...
                    for secret in secrets:
                        ...
                        if secret_type == "application/x-pkcs12":
                            print(
                                f"Writing certificate for {secret_name} to {secret_name}.pfx"
                            )
                            ...
                            if isinstance(secret_value_text, bytes):
                                with open(
                                    f"certificate/{resource_group_name}/{secret_name}.pfx",
                                    "wb",
                                ) as file:
                    ...
                    if "ForbiddenByPolicy" in str(e):
                        message.append(
                            "Error accessing secrets: Access Policy required for Secrets"
                        )
                    elif "ForbiddenByRbac" in str(e):
                        message.append(
                            "You do not have access to this Key Vault: RBAC required"
                        )
                    elif "AccessDenied" in str(e):
                        message.append(
                            "You do not have access to this Key Vault: Access Policy required"
                        )
                ...
                try:
                    ...
                    for key in keys:
                        ...
                        if key_type == "RSA":
                            usable_jwk = {
                                k: urlsafe_b64encode(vars(key_data.key)[k])
                                if isinstance(vars(key_data.key)[k], bytes)
                                else vars(key_data.key)[k]
                                for k in vars(key_data.key)
                                if vars(key_data.key)[k]
                            }
                        ...
                        if key_type == "EC":
                            usable_jwk = {
                                k: urlsafe_b64encode(vars(key_data.key)[k]).decode(
                                    "utf-8"
                                )
                                if isinstance(vars(key_data.key)[k], bytes)
                                else vars(key_data.key)[k]
                        ...
                        keys_list.append(
                            {"key_name": key_name, "key_value": f"{key_data_text}"}
                        )

                except Exception as e:
                    if "ForbiddenByPolicy" in str(e):
                        message.append(
                            "Error accessing keys: Access Policy required for Keys and Certificates"
                        )
                    elif "ForbiddenByRbac" in str(e):
                        pass
