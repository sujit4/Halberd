...
class AzureEnumerateRoleAssignment(BaseTechnique):
    def __init__(self):
        ...
        super().__init__(
            ...
            "Enumerates Azure RBAC role assignments to discover permissions granted to identities (users, groups, service principals) within the target subscription. This technique helps map out the access control landscape by revealing: who has access (users, groups, service principals), what level of access they have (role definitions), where they have access (scope of assignments)",
            mitre_techniques,
        )
    ...
    def execute(self, **kwargs: Any) -> Tuple[ExecutionStatus, Dict[str, Any]]:
        ...
        try:
            ...
            if scope:
                role_assignments_list = mgmt_client.role_assignments.list_for_scope(
                    scope=scope
                )
            else:
                role_assignments_list = (
                    mgmt_client.role_assignments.list_for_subscription()
                )
            ...
            for role_assignment in role_assignments_list:
                ...
                if (
                    principal_type
                    and assignment_dict.get("principal_type") != principal_type
                ):
                    continue
