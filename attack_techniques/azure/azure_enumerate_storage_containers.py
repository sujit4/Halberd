...
class AzureEnumerateStorageContainers(BaseTechnique):
    def __init__(self):
        ...
        technique_notes = [
            TechniqueNote(
                "The technique requires Storage Account Contributor or higher role to access the storage account keys"
            ),
            TechniqueNote(
                "Use file type analysis to identify containers with potentially sensitive content (e.g., databases, backups, configuration files)"
            ),
            TechniqueNote(
                "Monitor containers with public access enabled as they may expose data without authentication"
            ),
            TechniqueNote(
                "Large containers with recent modifications could indicate active data stores worth targeting"
            ),
            TechniqueNote(
                "Containers with immutability policies or legal holds may contain business-critical or compliance-related data"
            ),
            TechniqueNote(
                "The technique handles access denied scenarios, allowing identification of containers where additional privileges are needed"
            ),
        ]
        technique_refs = [
            TechniqueReference(
        ...
        def _get_container_stats(container_client: ContainerClient) -> Dict[str, Any]:
            ...
            try:
                ...
                return {
                    ...
                    "last_modified_blob": max(
                        (blob.last_modified for blob in blob_list), default=None
                    ),
                    "file_types": list(
                        set(
                            blob.name.split(".")[-1].lower()
