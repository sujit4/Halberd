...
class AzureEnumerateLogicApps(BaseTechnique):
    def __init__(self):
        ...
        technique_notes = [
            TechniqueNote(
                "Focus on Logic Apps with HTTP triggers for potential entry points"
            ),
            TechniqueNote(
                "Examine workflow definitions for hardcoded credentials and secrets"
            ),
            TechniqueNote(
                "Look for Logic Apps that connect to other Azure resources like Key Vault"
            ),
            TechniqueNote(
                "Logic Apps can be targeted for persistence mechanisms in Azure environments"
            ),
            TechniqueNote(
                "Try narrowing enumeration to a resource group or using other filters for faster results"
            ),
        ]
        super().__init__(
    ...
    def execute(self, **kwargs: Any) -> Tuple[ExecutionStatus, Dict[str, Any]]:
        ...
        try:
            ...
            if resource_group_name:
                ...
                logic_apps.append(
                    {"resource_group": resource_group_name, "logic_apps": workflow_list}
                )
                ...
                for rg in resource_groups:
                    try:
                        ...
                        if workflow_list:
                            logic_apps.append(
                                {"resource_group": rg.name, "logic_apps": workflow_list}
                            )
                            total_apps += len(workflow_list)
                    except Exception:
                        continue
        ...
        if workflow_name:
            try:
                ...
                if not state_filter or workflow.state == state_filter:
                    workflow_info = self._extract_workflow_info(
                        workflow, include_definitions, logic_client, resource_group_name
                    )
            ...
            workflows = logic_client.workflows.list_by_resource_group(
                resource_group_name
            )
    ...
    def _extract_workflow_info(
        ...
        workflow_info = {
            ...
            "created_time": str(workflow.created_time)
            if workflow.created_time
            else None,
            "changed_time": str(workflow.changed_time)
            if workflow.changed_time
            else None,
            ...
            for trigger_name, trigger_info in definition["triggers"].items():
                ...
                if trigger_type == "Request" or "Http" in trigger_type:
                    triggers[trigger_name] = {
                        ...
                        "method": trigger_info.get("inputs", {}).get(
                            "method", "Unknown"
                        ),
                    ...
                    triggers[trigger_name] = {
                        "type": trigger_type,
                        "recurrence": trigger_info.get("recurrence", "N/A")
                    ...
                    if isinstance(conn_value, dict):
                        ...
                        if "connectionId" in conn_value:
                            connections[conn_name] = {
                                ...
                                "connection_name": conn_value.get(
                                    "connectionName", "Unknown"
                                ),
                                "connection_type": conn_value.get(
                                    "id", "Unknown"
                                ).split("/")[-1]
                                if "id" in conn_value
                                else "Unknown",
