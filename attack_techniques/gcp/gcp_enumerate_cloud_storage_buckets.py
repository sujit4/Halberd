...
class GCPEnumerateCloudStorageBuckets(BaseTechnique):
    def __init__(self):
        ...
        technique_notes = [
            TechniqueNote(
                "Project filtering requires Storage Admin role on the project level"
            ),
            TechniqueNote(
                "Location filtering uses bucket's geographical placement, not data residency"
            ),
            TechniqueNote(
                "Storage Classes available: STANDARD, NEARLINE, COLDLINE, ARCHIVE"
            ),
            TechniqueNote(
                "Public buckets include both allUsers and allAuthenticatedUsers permission"
            ),
            TechniqueNote(
                "IAM policy checks require additional storage.buckets.getIamPolicy permission"
            ),
        ...
        technique_refs = [
            ...
            TechniqueReference(
                "GCP Bucket Naming Requirements",
        ...
        super().__init__(
            ...
            description=(
                "Performs comprehensive storage bucket enumeration across GCP projects. "
        ...
        try:
            ...
            if project_id:
                storage_client = storage.Client(
                    project=project_id, credentials=credential
                )
            else:
            ...
            try:
                ...
                for bucket in bucket_iterator:
                    ...
                    if (
                        storage_class
                        and bucket.storage_class.lower() != storage_class.lower()
                    ):
                        continue
                    ...
                    if label_key and label_value:
                        if (
                            not bucket.labels
                            or label_key not in bucket.labels
                            or bucket.labels[label_key] != label_value
                        ):
                            continue
                    elif label_key:
                    ...
                    bucket_info = {
                        ...
                        if hasattr(bucket, "retention_period")
                        else None,
                    ...
                    if only_public:
                        if (
                            bucket_info["public_access"]["public_roles"]
                            and bucket_info["public_access"]["public_roles"]
                            != "Access Denied"
                        ):
                        ...
                        "value": {
                            ...
                            "summary": {
                                "public_buckets": sum(
                                    1
                                    for b in buckets
                                    if b["public_access"]["public_roles"]
                                    and b["public_access"]["public_roles"]
                                    != "Access Denied"
                                ),
                                "versioned_buckets": sum(
                                    1 for b in buckets if b["versioning_enabled"]
                                ),
                                "locations": list(set(b["location"] for b in buckets)),
                                "storage_classes": list(
                                    set(b["class"] for b in buckets)
                                ),
                            },
                            "buckets": buckets,
                        },
