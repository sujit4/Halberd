...
class AWSEnumerateEC2Instances(BaseTechnique):
    def __init__(self):
        ...
        super().__init__(
            ...
            "Enumerates instances in AWS EC2. Optionally, pass additional parameters to filter results",
            mitre_techniques,
        )
        ...
        try:
            ...
            for aws_region in regions_to_enumerate:
                try:
                    ...
                    if max_results:
                        raw_response = my_client.describe_instances(
                            Filters=filters, DryRun=dry_run, MaxResults=max_results
                        )
                    else:
                        raw_response = my_client.describe_instances(
                            Filters=filters, DryRun=dry_run
                        )
            ...
            return ExecutionStatus.SUCCESS, {
                "message": f"Successfully enumerated {total_instances} EC2 instances"
                if output
                else "No EC2 instances found",
                "value": output,
            }
