...
class AWSEnumerateIAMPolicies(BaseTechnique):
    def __init__(self):
        ...
        super().__init__(
            ...
            "Enumerates all IAM policies. Optionally, supply scope & path prefix to enumerate specific policies",
            mitre_techniques,
        )
        ...
        try:
            ...
            if path_prefix in [None, ""]:
                if scope in [None, ""]:
                    ...
                    raw_response = my_client.list_policies(Scope=scope)
            else:
                if scope in [None, ""]:
                    # list policies in aws account with specified path prefix
                    raw_response = my_client.list_policies(PathPrefix=path_prefix)
                else:
                    # list policies in aws account with specified path prefix & scope
                    raw_response = my_client.list_policies(
                        PathPrefix=path_prefix, Scope=scope
                    )
            ...
            if 200 <= raw_response["ResponseMetadata"]["HTTPStatusCode"] < 300:
                return ExecutionStatus.SUCCESS, {
                    "message": f"Successfully enumerated {len(policies)} users"
                    if policies
                    else "No users found",
                    "value": policies,
                }
