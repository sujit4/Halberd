...
class AWSReconIAMUserInfo(BaseTechnique):
    def __init__(self):
        ...
        super().__init__(
            "Recon IAM User Info",
            "Retrieves information about a user in AWS",
            mitre_techniques,
        )
        ...
        try:
            ...
            if 200 <= raw_response["ResponseMetadata"]["HTTPStatusCode"] < 300:
                return ExecutionStatus.SUCCESS, {
                    ...
                    "value": {
                        "username": raw_response.get("User", "N/A").get(
                            "UserName", "N/a"
                        ),
                        "user_id": raw_response.get("User", "N/A").get("UserId", "N/A"),
                        "arn": raw_response.get("User", "N/A").get("Arn", "N/A"),
                        "create_date": raw_response.get("User", "N/A").get(
                            "CreateDate", "N/A"
                        ),
                        "password_last_used": raw_response.get("User", "N/A").get(
                            "PasswordLastUsed", "N/A"
                        ),
                        "permissions_boundary": raw_response.get("User", "N/A").get(
                            "PermissionsBoundary", "N/A"
                        ),
