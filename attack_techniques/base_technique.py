...
class BaseTechnique(ABC):
    def __init__(
        ...
        azure_trm_techniques: List[AzureTRMTechnique] = None,
        references: List[TechniqueReference] = None,
        notes: List[TechniqueNote] = None,
    ):
        self.name = name
    ...
    def validate_parameters(self, params: Dict[str, Any]) -> None:
        ...
        for param, value in params.items():
            ...
            if param_type and isinstance(value, eval(param_type)):
                ...
                if value:
                    raise TypeError(
                        f"Invalid type for parameter {param}. Expected {param_type}, got {type(value)}"
                    )
                else:
                    pass
