...
class EntraReconUserInfo(BaseTechnique):
    ...
    def execute(self, **kwargs: Any) -> Tuple[ExecutionStatus, Dict[str, Any]]:
        ...
        try:
            ...
            if basic_info_success:
                ...
                results["basic_info"] = {
                    "error": basic_info.get(
                        "error", "Failed to retrieve basic user information"
                    )
                }
            ...
            if groups_success:
                ...
                results["group_memberships"] = {
                    "error": groups_info.get(
                        "error", "Failed to retrieve group memberships"
                    )
                }
            ...
            if roles_success:
                ...
                results["directory_roles"] = {
                    "error": roles_info.get(
                        "error", "Failed to retrieve directory roles"
                    )
                }
            ...
            if apps_success:
                ...
                results["app_assignments"] = {
                    "error": apps_info.get(
                        "error", "Failed to retrieve application assignments"
                    )
                }
            ...
            if manager_success:
                ...
                results["manager_info"] = {
                    "error": manager_info.get(
                        "error", "Failed to retrieve manager information"
                    )
                }

            # 6. Get user's direct reports
            reports_success, reports_info = self._get_user_direct_reports(
                user_identifier
            )
            if reports_success:
                ...
                results["direct_reports"] = {
                    "error": reports_info.get(
                        "error", "Failed to retrieve direct reports"
                    )
                }
            ...
            if owned_success:
                ...
                results["owned_objects"] = {
                    "error": owned_info.get("error", "Failed to retrieve owned objects")
                }

            # 8. Get user's registered devices
            devices_success, devices_info = self._get_user_registered_devices(
                user_identifier
            )
            if devices_success:
                ...
                results["registered_devices"] = {
                    "error": devices_info.get(
            ...
            if not failed_operations:
                return ExecutionStatus.SUCCESS, {
                    "message": "Successfully gathered complete user reconnaissance data",
                    "value": results,
                }
                ...
                return ExecutionStatus.FAILURE, {
                    "error": results,
                    "message": "Failed to gather user reconnaissance data",
                }
    ...
    def _get_basic_user_info(self, user_identifier: str) -> Tuple[bool, Dict[str, Any]]:
        ...
        try:
            ...
            if "error" in raw_response:
                return False, {
                    "error": f"Error {raw_response.get('error').get('code')}: {raw_response.get('error').get('message')}"
                }
            ...
            user_info = {
                ...
                "last_password_change": raw_response.get(
                    "lastPasswordChangeDateTime", "N/A"
                ),
                "password_policies": raw_response.get("passwordPolicies", "N/A"),
                "assigned_licenses": [
                    license.get("skuId", "N/A")
                    for license in raw_response.get("assignedLicenses", [])
                ],
    ...
    def _get_user_groups(self, user_identifier: str) -> Tuple[bool, Dict[str, Any]]:
        ...
        try:
            ...
            return True, {"total_groups": len(groups), "groups": groups}

        except Exception as e:
            return False, {"error": str(e)}

    def _get_user_directory_roles(
        ...
        try:
            ...
            directory_roles = []
            if "error" not in dir_roles_response:
    ...
    def _get_user_direct_reports(
        ...
        try:
            ...
            return True, {"total_direct_reports": len(reports), "reports": reports}

        except Exception as e:
            return False, {"error": str(e)}

    def _get_user_owned_objects(
        self, user_identifier: str
    ) -> Tuple[bool, Dict[str, Any]]:
    ...
    def _get_user_registered_devices(
        self, user_identifier: str
    ) -> Tuple[bool, Dict[str, Any]]:
        ...
        try:
            ...
            return True, {"total_registered_devices": len(devices), "devices": devices}
