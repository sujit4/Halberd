...
class EntraReconRoleInfo(BaseTechnique):
    def __init__(self):
        ...
        technique_notes = [
            TechniqueNote(
                "This technique works with both active directory roles and role templates - specify either a role name (e.g., 'Global Administrator') or a role ID/template ID"
            ),
            TechniqueNote(
                "Some information may require higher privileges - the technique will indicate where data could not be retrieved due to insufficient permissions"
            ),
        ]
        ...
        technique_refs = [
            ...
            TechniqueReference(
                "Microsoft Graph API - Directory Roles",
                "https://docs.microsoft.com/en-us/graph/api/resources/directoryrole",
            ),
        ]
    ...
    def execute(self, **kwargs: Any) -> Tuple[ExecutionStatus, Dict[str, Any]]:
        ...
        try:
            ...
            assignments_result = self._get_role_assignments(
                role_info.get("roleTemplateId", role_info.get("id"))
            )
            recon_results["role_assignments"] = assignments_result["assignments"]

            # Step 4: Check administrative unit assignments
            admin_units_result = self._get_administrative_unit_assignments(
                role_info.get("roleTemplateId")
            )
            recon_results["administrative_units"] = admin_units_result["admin_units"]

            # Step 5: Get PIM eligible assignments (if requested)
            if include_pim_data:
                pim_result = self._get_pim_eligible_assignments(
                    role_info.get("roleTemplateId")
                )
                recon_results["pim_eligible_assignments"] = pim_result[
                    "eligible_assignments"
                ]

            # Step 6: Analyze role permissions and scope
            permissions_result = self._analyze_role_permissions(
                role_info.get("roleTemplateId")
            )
            ...
            successful_sections = sum(
                [
                    ...
                    1
                    if recon_results["pim_eligible_assignments"] or not include_pim_data
                    else 0,
    ...
    def _identify_target_role(self, role_input: str) -> Dict[str, Any]:
        ...
        try:
            ...
            endpoint_url = (
                f"https://graph.microsoft.com/v1.0/directoryRoles/{role_input}"
            )
        ...
        try:
            ...
            if "error" not in response and response:
                for template in response:
                    if (
                        template.get("displayName", "").lower() == role_input.lower()
                        or template.get("id") == role_input
                    ):
                        return {
    ...
    def _get_role_members(self, role_id: str) -> Dict[str, Any]:
        ...
        try:
                    ...
                    members.append(
                        {
                            ...
                            "objectType": member.get("@odata.type", "N/A").replace(
                                "#microsoft.graph.", ""
                            ),
            ...
            return {"members": [], "error": f"Exception getting role members: {str(e)}"}
    ...
    def _get_role_assignments(self, role_template_id: str) -> Dict[str, Any]:
        ...
        try:
                    ...
                    assignments.append(
                        {
                            ...
                            "directoryScopeId": assignment.get(
                                "directoryScopeId", "N/A"
                            ),
                            "appScopeId": assignment.get("appScopeId", "N/A"),
                            "assignmentType": assignment.get(
            ...
            return {"assignments": assignments, "assignment_count": len(assignments)}
    ...
    def _get_administrative_unit_assignments(
        self, role_template_id: str
    ) -> Dict[str, Any]:
    ...
    def _analyze_role_permissions(self, role_template_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"permissions": [permissions_info]}
    ...
    def _get_principal_details(self, principal_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"type": "error", "id": principal_id, "error": str(e)}

    def _get_administrative_unit_details(self, au_id: str) -> Dict[str, Any]:
        """Get administrative unit details"""
        try:
            endpoint_url = (
                f"https://graph.microsoft.com/v1.0/administrativeUnits/{au_id}"
            )
            response = GraphRequest().get(url=endpoint_url)
            ...
            return {"error": f"Exception getting AU details: {str(e)}"}

    def _generate_privilege_analysis(
