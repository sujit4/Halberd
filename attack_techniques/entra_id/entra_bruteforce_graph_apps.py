...
class EntraBruteforceGraphApps(BaseTechnique):
    def __init__(self):
        mitre_techniques = [
            MitreTechnique(
        ...
        super().__init__(
            ...
            "Bruteforce through default Azure apps using user credentials to collect access tokens with different scopes",
            mitre_techniques,
        )
    ...
    def execute(self, **kwargs: Any) -> Tuple[ExecutionStatus, Dict[str, Any]]:
        ...
        try:
            ...
            save_tokens: bool = kwargs.get("save_tokens", False)
            ...
            default_azure_applications_list = [
            ...
            for app in default_azure_applications_list:
                ...
                raw_response = requests.post(
                    url=endpoint_url, headers=headers, data=data
                ).json()
                if 200 <= raw_response.status_code < 300:
                    access_token = raw_response["access_token"]
                    success_auth.append({"app": app, "access_token": access_token})
                    if save_tokens:
                        # save access token
            ...
            return ExecutionStatus.SUCCESS, {
                "message": f"Successfully collected {len(success_auth)} app tokens",
                "value": {"collected_tokens": success_auth, "failed_auth": failed_auth},
            }

        except Exception as e:
            return ExecutionStatus.FAILURE, {
                "error": str(e),
                "message": "Failed to execute graph app bruteforce",
            }
