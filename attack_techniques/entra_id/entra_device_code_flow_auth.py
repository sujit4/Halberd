...
def AcquireDeviceCodeFlowToken(token_url, token_data, polling_interval):
    ...
    while True:
        ...
        if token_response.status_code == 200:
            ...
            EntraTokenManager().add_token(
                access_token=access_token, refresh_token=refresh_token
            )
            break
        elif (
            token_response.status_code == 400
            and "error" in token_response.json()
...
class EntraDeviceCodeFlowAuth(BaseTechnique):
    def __init__(self):
        ...
        super().__init__(
            ...
            "Authenticates to target tenant using Entra ID device code flow",
            mitre_techniques,
    ...
    def execute(self, **kwargs: Any) -> Tuple[ExecutionStatus, Dict[str, Any]]:
        ...
        try:
            ...
            if client_id in [None, ""]:
                client_id = (
                    "d3590ed6-52b3-4102-aeff-aad2292ab01c"  # set default client id
                )
            ...
            data = {
                ...
                "scope": scope
                + " "
                + "offline_access",  # offline_access to request refresh token
            }

            generate_device_code_flow = requests.post(
                url=endpoint_url, data=data
            ).json()
            ...
            raw_response = Process(
                target=AcquireDeviceCodeFlowToken,
                args=(token_url, token_data, polling_interval),
            )
            ...
            if user_code:
                return ExecutionStatus.SUCCESS, {
                    "message": "Successfully initialized device code flow auth",
                    "value": {
