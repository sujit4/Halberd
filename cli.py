...
def list_techniques(
    category: str = None, tactic: str = None, technique_id: str = None
) -> List[Dict]:
    """
    ...
    for tech_name, tech_class in all_techniques.items():
        ...
        if (
            category
            and not technique_registry.get_technique_category(tech_name) == category
        ):
            continue
        ...
        if tactic:
            tactic_match = any(
                tactic.lower() in mt.tactics for mt in technique.mitre_techniques
            )
            if not tactic_match:
                continue
        ...
        if technique_id:
            id_match = any(
                technique_id.upper() == mt.technique_id
...
def main():
    ...
    list_techniques_parser = subparsers.add_parser(
        "list-techniques", help="List techniques"
    )
    list_techniques_parser.add_argument(
        "--category",
        choices=["azure", "entra_id", "aws", "m365"],
        help="Filter techniques by category",
    )
    list_techniques_parser.add_argument(
        "--tactic", help="Filter techniques by MITRE tactic"
    )
    list_techniques_parser.add_argument(
        "--technique-id", help="Filter techniques by MITRE technique ID"
    )

    # List tactics command
    list_tactics_parser = subparsers.add_parser(
        "list-tactics", help="List MITRE tactics"
    )
    list_tactics_parser.add_argument(
        "--category",
        choices=["azure", "entra_id", "aws", "m365"],
        help="Filter tactics by category",
    )

    # Get technique category command
    get_category_parser = subparsers.add_parser(
        "get-category", help="Get category of a technique"
    )
    ...
    execute_parser.add_argument(
        "--params", type=json.loads, help="Technique parameters in JSON format"
    )
    ...
    if args.command == "list-techniques":
        ...
        try:
            ...
            print(json.dumps({"status": status.value, "result": result}, indent=2))
        except (ValueError, TypeError) as e:
            print(
                json.dumps({"status": "error", "result": {"error": str(e)}}, indent=2)
            )
    else:
        parser.print_help()
