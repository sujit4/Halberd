...
def list_techniques(
    attack_surface: str = None, tactic: str = None, technique_id: str = None
) -> List[Dict]:
    """
    ...
    for tech_id, tech_class in all_techniques.items():
        ...
        if (
            attack_surface
            and not technique_registry.get_technique_category(tech_id) == attack_surface
        ):
            continue
        ...
        tech_info = {"name": technique.name, "t_id": tech_id}
...
def execute_technique(t_id: str, technique_input: dict):
    ...
    if isinstance(output, tuple) and len(output) == 2:
        ...
        output_data = (
            response["value"] if result.value == "success" else response["error"]
        )
        # Save output to file
        output_file_path = output_manager.store_technique_output(
            data=output_data, technique_name=t_id, event_id=event_id
        )
