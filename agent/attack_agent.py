...
from .tools import (
    aws_get_active_session,
    aws_get_all_sessions,
    aws_get_connected_user_details,
    aws_get_session_details,
    aws_retrieve_sessions,
    aws_set_active_session,
    entra_id_decode_jwt_token,
    entra_id_get_active_token,
    entra_id_get_active_token_pair,
    entra_id_get_all_tokens,
    entra_id_set_active_token,
    execute_technique,
    get_app_info,
    get_technique_aztrm_info,
    get_technique_execution_response,
    get_technique_inputs,
    get_technique_mitre_info,
    list_tactics,
    list_techniques,
    read_halberd_logs,
    tools,
...
class RateLimiter:
    ...
    def _clean_old_records(self, current_time):
        ...
        self.requests_timestamps = [
            t for t in self.requests_timestamps if t > minute_ago
        ]

        # Clean token usage records
        self.input_token_usage = [
            (t, count) for t, count in self.input_token_usage if t > minute_ago
        ]
        self.output_token_usage = [
            (t, count) for t, count in self.output_token_usage if t > minute_ago
        ]
    ...
    def get_required_delay(self, input_tokens, est_output_tokens):
        ...
        output_token_delay = self._calculate_token_delay(
            est_output_tokens,
            self.output_token_usage,
            OUTPUT_RATE_LIMIT_TOKENS_PER_MIN,
            current_time,
        )
        ...
        if max_delay > 0:
            ...
            print(
                f"Rate limiting: Delay of {max_delay:.2f}s required due to {reason} limit"
            )
...
class AttackAgent:
    ...
    def anthropic(self):
        ...
        if self._anthropic_client is None or current_api_key != self._last_api_key:
            if current_api_key:
                try:
    ...
    def count_tokens_for_messages(self, messages, include_system=True):
        ...
        try:
            ...
            if include_system:
                ...
                count_result = self.anthropic.messages.count_tokens(
                    model="claude-3-7-sonnet-20250219", messages=messages_for_counting
                )
    ...
    def truncate_messages_history(self, messages, max_tokens):
        ...
        self.count_tokens_for_messages(
            recent_messages, include_system=True
        )
        ...
        for message in reversed(messages[:-1]):
            ...
            temp_token_count = self.count_tokens_for_messages(
                temp_messages, include_system=True
            )
    ...
    def generate_message(self, messages, max_tokens):
        ...
        if total_tokens > MAX_TOTAL_TOKENS - max_tokens:
            ...
            messages = self.truncate_messages_history(
                messages, MAX_TOTAL_TOKENS - max_tokens
            )
            # Recalculate tokens after truncation
            new_total_tokens = self.count_tokens_for_messages(
                messages, include_system=True
            )
        ...
        required_delay = self.rate_limiter.get_required_delay(
        ...
        if required_delay > 0:
            ...
            if (
                time_since_last_call < MIN_DELAY_BETWEEN_CALLS
                and self.last_api_call_time > 0
            ):
                minimum_delay = MIN_DELAY_BETWEEN_CALLS - time_since_last_call
                time.sleep(minimum_delay)
                print(
                    f"Enforcing minimum delay: {minimum_delay:.2f}s between API calls"
                )
        ...
        for attempt in range(max_retries):
            try:
                ...
                self.rate_limiter.record_usage(
                    total_tokens, actual_output_tokens, call_start_time
                )
                ...
                if "rate_limit_error" in error_str or "429" in error_str:
                    ...
                    if attempt == max_retries - 1:
                        return {
                            "error": f"Rate limit exceeded after {max_retries} attempts. Please try again later."
                        }

                    # Calculate backoff delay with jitter (randomness)
                    delay = base_delay * (2**attempt) + random.uniform(0, 1)
                    print(
                        f"Rate limit hit. Retrying in {delay:.2f} seconds (attempt {attempt + 1}/{max_retries})..."
                    )
                    time.sleep(delay)
                    ...
                    if hasattr(self, "rate_limiter"):
                        ...
                        self.rate_limiter.record_usage(
                            cautious_input_tokens, cautious_output_tokens, now - 1
                        )
                else:
    ...
    def process_user_input(self, user_input):
        ...
        try:
            ...
            while any(
                content.type == "tool_use" for content in current_message.content
            ):
